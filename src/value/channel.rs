use crate::util::{decode_crockford_base32_128, encode_crockford_base32_128};
...
impl ChannelId {
    pub(crate) fn parse(raw: &str) -> ValueResult<Self> {
        ...
        if trimmed.is_empty() {
            return Err(ValueError::new("channel id must not be empty"));
        }
        decode_crockford_base32_128(trimmed)
            .map(Self)
            .map_err(|_| ValueError::new("invalid channel id"))
    }
...
impl<'a> ChannelAlias<'a> {
    pub(crate) fn parse(raw: &'a str) -> ValueResult<Self> {
        let trimmed = raw.trim();
        if trimmed.is_empty() {
            return Err(ValueError::new("channel name must not be empty"));
        }
        let len = trimmed.chars().count();
        if len > MAX_CHANNEL_ALIAS_LEN {
            return Err(ValueError::new("channel name too long (max 128)"));
        }
        if trimmed.chars().any(|ch| ch.is_control()) {
            return Err(ValueError::new("channel name contains invalid characters"));
        }
        Ok(Self(trimmed))
    }
