...
use super::outbox::mark_provider_delivery_consumed_pg_tx;
...
impl PostgresDb {
    ...
    pub(super) async fn enqueue_provider_pull_item(
        ...
        let terminal: i64 = sqlx::query_scalar(
            "SELECT COUNT(1) FROM private_outbox WHERE device_id = $1 AND delivery_id = $2 AND status = $3",
        )
    ...
    pub(super) async fn pull_provider_item(
        ...
        let result = if let Some(r) = row {
            ...
            {
                mark_provider_delivery_consumed_pg_tx(
                    &mut tx,
                    device_id,
            ...
            .execute(&mut *tx)
            .await?;
            mark_provider_delivery_consumed_pg_tx(&mut tx, device_id, delivery_id, now).await?;
    ...
    pub(super) async fn ack_provider_items(
        ...
        separated.push_unseparated(") ORDER BY q.delivery_id ASC FOR UPDATE OF q");
        ...
        for candidate in candidates {
            let result = sqlx::query(
                "DELETE FROM provider_pull_queue \
                 WHERE device_id = $1 AND delivery_id = $2 AND expires_at > $3 \
                   AND COALESCE( \
            ...
            if result.rows_affected() == 1 {
                delete_orphan_private_payload_in_pg_tx(&mut tx, &candidate.delivery_id).await?;
