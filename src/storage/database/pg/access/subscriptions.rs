...
impl PostgresDb {
    ...
    pub(super) async fn update_channel_password_hash(
        ...
        sqlx::query(
            "UPDATE channels SET password_hash = $1, updated_at = $2 WHERE channel_id = $3",
        )
        .bind(password_hash)
        .bind(Utc::now().timestamp_millis())
        .bind(&channel_id[..])
    ...
    pub(super) async fn private_unsubscribe_channel(
        ...
        sqlx::query("DELETE FROM channel_subscriptions WHERE channel_id = $1 AND device_id = $2")
            .bind(&channel_id[..])
            .bind(&device_id[..])
            .execute(&self.pool)
            .await?;
    ...
    pub(super) async fn list_private_subscribers(
        ...
        let rows = sqlx::query(
            "SELECT s.device_id FROM channel_subscriptions s \
             JOIN devices d ON d.device_id = s.device_id \
             WHERE s.channel_id = $1 AND s.status = 'active' AND d.channel_type = 'private' AND d.route_updated_at IS NOT NULL AND s.created_at <= $2 \
