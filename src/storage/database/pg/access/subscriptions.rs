...
fn route_matches_dispatch_target(
    ...
    let Some(route_channel_type) = normalize_optional_token(route_channel_type) else {
        return false;
    };
    ...
    let Some(subscription_provider_token) = normalize_optional_token(subscription_provider_token)
    else {
        return false;
    };
