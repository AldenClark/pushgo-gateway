...
fn thing_route_action_name(action: ThingRouteAction) -> &'static str {
    match action {
        ThingRouteAction::Create => "create",
        ThingRouteAction::Update => "update",
        ThingRouteAction::Archive => "archive",
        ThingRouteAction::Delete => "delete",
    }
}
...
async fn thing_to_channel_with_action(
    ...
    let action_name = thing_route_action_name(route_action);
    ...
    let fut = async move {
                ...
                .is_some_and(|value| !value.trim().is_empty())
        {
            ::tracing::event!(
                ...
                event = "thing.route_rejected",
                action = %(action_name),
                reason = %("thing_id_forbidden_on_create")
            );
            return Err(Error::validation_code(
                "thing_id is generated by gateway on /thing/create",
                "thing_id_forbidden_on_create",
            ));
        }
        let resolved_thing_id = match route_action {
            ...
            ThingRouteAction::Update | ThingRouteAction::Archive | ThingRouteAction::Delete => {
                ResolvedSemanticId {
                    semantic_id: {
                        let raw = payload.thing_id.as_deref().ok_or_else(|| {
                            ::tracing::event!(
                                target: "gateway.trace_event",
                                ::tracing::Level::WARN,
                                event = "thing.route_rejected",
                                action = %(action_name),
                                reason = %("thing_id_required")
                            );
                            Error::validation_code("thing_id is required", "thing_id_required")
                        })?;
        ...
        let observed_at = payload.payload.mutable.observed_at.ok_or_else(|| {
            ::tracing::event!(
                target: "gateway.trace_event",
                ::tracing::Level::WARN,
                event = "thing.route_rejected",
            ...
            ThingRouteAction::Update | ThingRouteAction::Archive | ThingRouteAction::Delete => {
                if payload.payload.created_at.is_some() {
                    ::tracing::event!(
                        ...
                        event = "thing.route_rejected",
                        action = %(action_name),
                        reason = %("created_at_forbidden_on_update")
                    );
                    return Err(Error::validation_code(
                        "created_at is only allowed on /thing/create",
                        "created_at_forbidden_on_update",
                    ));
                }
                None
            }
        ...
        let final_attrs_json = serde_json::to_string(&final_attrs).map_err(|_| {
        ...
        let dispatch_summary = {
            ...
            extra.insert("attrs".to_string(), final_attrs_json.clone());
            if let Some(profile) = merged_profile.as_ref() {
                ...
                if !profile.external_ids.is_empty()
                    && let Ok(serialized) = serde_json::to_string(&profile.external_ids)
                {
