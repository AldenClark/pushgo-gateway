...
use super::audit::{DispatchAuditLog, DispatchAuditRecord};
use crate::storage::{DeliveryAuditWrite, Storage};
...
pub(crate) enum DeliveryAuditMode {
    Enabled,
    Disabled,
}

impl DeliveryAuditMode {
...
impl DeliveryAuditCollector {
    pub(crate) fn spawn(
        ...
        if !mode.is_enabled() {
            return Arc::new(Self {
                mode: DeliveryAuditMode::Disabled,
                tx: None,
            });
        }
        ...
        Arc::new(Self {
            mode: DeliveryAuditMode::Enabled,
            tx: Some(tx),
        })
    }
...
mod tests {
    use super::*;
    use crate::dispatch::audit::DispatchAuditMode;
    use crate::storage::{DeliveryAuditPath, DeliveryAuditStatus, Storage};
    ...
    async fn collector_disabled_skips_writes() {
        ...
        let audit = Arc::new(DispatchAuditLog::new(32, DispatchAuditMode::Enabled));
        let collector =
            DeliveryAuditCollector::spawn(DeliveryAuditMode::Disabled, store.clone(), audit);
    ...
    async fn collector_enabled_flushes_batch_to_storage() {
        ...
        let audit = Arc::new(DispatchAuditLog::new(32, DispatchAuditMode::Enabled));
        let collector =
            DeliveryAuditCollector::spawn(DeliveryAuditMode::Enabled, store.clone(), audit);
